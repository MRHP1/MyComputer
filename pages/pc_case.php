...
// Handle adding pc_case to user's profile
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pc_case_id'])) {
    $pc_case_id = intval($_POST['pc_case_id']);
    $user_id = $_SESSION['UID'];

    // Update the user's pc_case in the database
    $stmt = $conn->prepare("UPDATE users SET user_case = ? WHERE id = ?");
    $stmt->bind_param("ii", $pc_case_id, $user_id);
    if ($stmt->execute()) {
        ...
        echo "<p style='color: red;'>Failed to add pc_case. Please try again.</p>";
    }
    $stmt->close();
}
...
// Fetch the list of pc_cases
$pc_cases = [];
$sql = "SELECT * FROM pc_case" . $searchQuery;
    ...
    <div class="parts-container">
        <div class="search-bar">
            <form action="" method="GET">
                <input type="text" name="search" placeholder="Search Cases...">
        ...
        if (count($pc_cases) > 0) {
            foreach ($pc_cases as $row) {
                $imagePath = $row['image'] ?: "../images/pc_case/default.jpg";
            ...
            echo "<p>No PC pc_cases available.</p>";
        }
        ?>
    </div>
</body>
