...
// Handle adding cpu_cooler to user's profile
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cpu_cooler_id'])) {
    $cpu_cooler_id = intval($_POST['cpu_cooler_id']);
    $user_id = $_SESSION['UID'];

    // Update the user's cpu_cooler in the database
    $stmt = $conn->prepare("UPDATE users SET user_cooler = ? WHERE id = ?");
    $stmt->bind_param("ii", $cpu_cooler_id, $user_id);
    if ($stmt->execute()) {
        ...
        echo "<p style='color: red;'>Failed to add cpu_cooler. Please try again.</p>";
    }
    $stmt->close();
}
...
// Fetch the list of cpu_coolers
$cpu_coolers = [];
$sql = "SELECT * FROM cpu_cooler" . $searchQuery;
    ...
    <div class="parts-container">
        <div class="search-bar">
            <form action="" method="GET">
                <input type="text" name="search" placeholder="Search cpu_coolers...">
                <button type="submit">Search</button>
            </form>
        </div>
        <?php
        
        if (count($cpu_coolers) > 0) {
            foreach ($cpu_coolers as $row) {
                $imagePath = $row['image'] ?: "../images/cpu_cooler/default.jpg";

                echo '<div class="part">';
                echo '<img src="' . $imagePath . '" alt="' . $row['name'] . '">';
                echo '<h3>' . $row['name'] . '</h3>';
                echo '<ul class="specs">';
                echo '<li><strong>RPM:</strong> ' . $row['rpm'] . '</li>';
                echo '<li><strong>Noise:</strong> ' . $row['noise'] . '</li>';
                echo '<li><strong>Price:</strong> Rp ' . $row['price'] . '</li>';
                echo '<li><strong><a href="' . $row['link'] . '" target="_blank">View More</a></strong></li>';
                echo '</ul>';

                echo '<form action="" method="POST" style="display:inline;">';
                echo '<input type="hidden" name="cpu_cooler_id" value="' . $row['id'] . '">';
            ...
            echo "<p>No cpu_cooler cpu_coolers available.</p>";
        }
        ?>
    </div>
</body>
