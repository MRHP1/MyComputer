...
<body>
    ...
    <div class="parts-container">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["upload_image"])) {
            ...
            $targetDir = "../images/gpu/";
            $targetFile = $targetDir . basename($imageFile);

            if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFile)) {
                $updateImageSql = "UPDATE GPU SET image = '$targetFile' WHERE id = $gpuId";
                if ($conn->query($updateImageSql) === TRUE) {
                    echo "<p>Image uploaded successfully for GPU ID: $gpuId</p>";
                } else {
        ...
        // Fetch GPUs from the database
        $sql = "SELECT * FROM GPU";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                $imagePath = $row["image"] ?: "../images/gpu/default.jpg"; // Default image if none exists
                ...
                echo '<form action="gpu_edit.php" method="GET" style="display:inline;">';
                echo '<input type="hidden" name="id" value="' . $row["id"] . '">';
                echo '<button type="submit" class="edit">Edit</button>';
                echo '</form>';
                echo '<form action="gpu_delete.php" method="POST" style="display:inline;">';
                echo '<input type="hidden" name="id" value="' . $row["id"] . '">';
