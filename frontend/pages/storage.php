    ...
    <div class="parts-container">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["upload_image"])) {
            ...
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFile)) {
                $updateImageSql = "UPDATE Storage SET image = '$targetFile' WHERE id = $storageId";
                if ($conn->query($updateImageSql) === TRUE) {
                    echo "<p>Image uploaded successfully for Storage ID: $storageId</p>";
                } else {
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                $imagePath = $row["image"] ?: "../images/storage/default.jpg"; // Default image if none exists
                ...
                echo '<form action="storage_edit.php" method="GET" style="display:inline;">';
                echo '<input type="hidden" name="id" value="' . $row["id"] . '">';
                echo '<button type="submit" class="edit">Edit</button>';
                echo '</form>';
                echo '<form action="storage_delete.php" method="POST" style="display:inline;">';
                echo '<input type="hidden" name="id" value="' . $row["id"] . '">';
