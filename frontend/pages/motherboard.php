...
<body>
    ...
    <div class="parts-container">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["upload_image"])) {
            $motherboardId = $_POST["motherboard_id"];
            $imageFile = $_FILES["image"]["name"];
            $targetDir = "../images/motherboard/";
            $targetFile = $targetDir . basename($imageFile);

            if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFile)) {
                $updateImageSql = "UPDATE Motherboard SET image = '$targetFile' WHERE id = $motherboardId";
                if ($conn->query($updateImageSql) === TRUE) {
                    echo "<p>Image uploaded successfully for Motherboard ID: $motherboardId</p>";
                } else {
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                $imagePath = $row["image"] ?: "../images/motherboard/default.jpg"; // Default image if none exists
                ...
                echo '<form action="motherboard_edit.php" method="GET" style="display:inline;">';
                echo '<input type="hidden" name="id" value="' . $row["id"] . '">';
                echo '<button type="submit" class="edit">Edit</button>';
                echo '</form>';
                echo '<form action="motherboard_delete.php" method="POST" style="display:inline;">';
                echo '<input type="hidden" name="id" value="' . $row["id"] . '">';
