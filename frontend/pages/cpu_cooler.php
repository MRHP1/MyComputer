...
<body>
    ...
    <div class="parts-container">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["upload_image"])) {
            $cpu_coolerId = $_POST["cpu_cooler_id"];
            $imageFile = $_FILES["image"]["name"];
            $targetDir = "../images/cpu_cooler/";
            $targetFile = $targetDir . basename($imageFile);

            if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFile)) {
                $updateImageSql = "UPDATE CPU_Cooler SET image = '$targetFile' WHERE id = $cpu_coolerId";
                if ($conn->query($updateImageSql) === TRUE) {
                    echo "<p>Image uploaded successfully for CPU Cooler ID: $cpu_coolerId</p>";
                } else {
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                $imagePath = $row["image"] ?: "../images/cpu_cooler/default.jpg"; // Default image if none exists
                ...
                echo '<form action="cpu_cooler_edit.php" method="GET" style="display:inline;">';
                echo '<input type="hidden" name="id" value="' . $row["id"] . '">';
                echo '<button type="submit" class="edit">Edit</button>';
                echo '</form>';
                echo '<form action="cpu_cooler_delete.php" method="POST" style="display:inline;">';
                echo '<input type="hidden" name="id" value="' . $row["id"] . '">';
